...
#'
#' @export
imT2Sim <- function(data, indivdual, width = NULL, height = NULL, elementId = NULL) {
...
#'
#' @export
imT2SimOutput <- function(outputId, width = 'auto', height = '500px'){
  htmlwidgets::shinyWidgetOutput(outputId, 'imT2Sim', width, height, package = 'imRaceSim')
}

#' @rdname imSwimSim-shiny
#' @export
renderImT2Sim <- function(expr, env = parent.frame(), quoted = FALSE) {
  if (!quoted) { expr <- substitute(expr) } # force quoted
  htmlwidgets::shinyRenderWidget(expr, imT2SimOutput, env, quoted = TRUE)
}
